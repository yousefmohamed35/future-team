...
class _CoursesScreenState extends State<CoursesScreen> {
  ...
  void initState() {
    super.initState();
    _loadStudentGradeAndId();
  }

  Future<void> _loadStudentGradeAndId() async {
    final gradeStr =
        await SharedPrefHelper.getString(SharedPrefKeys.studentGrade);
    final id = await SharedPrefHelper.getString(SharedPrefKeys.userId);
    if (mounted) {
      setState(() {
        _studentGrade = int.tryParse(gradeStr ?? '44') ?? 44;
        _studentId = id ?? '';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => getIt<CoursesCubit>()
        ..getBanners()
        ..getCourses(filtersLevels: _studentGrade),
      child: _CoursesScreenContent(
...
class _CoursesScreenContentState extends State<_CoursesScreenContent> {
  ...
  /// Selected grade filter: 44=أولى, 52=ثانية, 59=ثالثة, 65=رابعة (API level ids)
  int? _selectedFiltersLevels;
  ...
  void initState() {
    ...
    _startAutoScroll();
    // Default selected grade is the student's own grade
    _selectedFiltersLevels = _studentGrade;
  }
  ...
  int get _studentGrade => widget.studentGrade;
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            ...
            await context.read<CoursesCubit>().getCourses(
                  refresh: true,
                  filtersLevels: _selectedFiltersLevels,
                );
          },
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Grade filter (أولى، ثانية، ثالثة، رابعة) – always visible without horizontal scroll
                _buildGradeFilter(),
  ...
  Widget _buildCoursesGrid() {
    return BlocBuilder<CoursesCubit, CoursesState>(
      builder: (context, state) {
        final cubit = context.read<CoursesCubit>();
        // مدفوعة فقط + تظهر فقط إذا أنت (الأدمن) ضفت الطالب في الكورس (assigned_student_ids من الباكند)
        final studentId = _studentId.trim();
        final hasValidStudentId = studentId.isNotEmpty;
        final courses = cubit.allCourses.where((course) {
          if (course.isFree) return false;
          // final int effectiveGrade = _selectedFiltersLevels ?? _studentGrade;
          // final gradeMatch =
          //     course.grade == null || course.grade == effectiveGrade;
          // لا تظهر الكورس إلا لو الطالب مضاف له من لوحة التحكم
          final isAssigned = hasValidStudentId &&
              course.assignedStudentIds
                  .any((id) => id.toString().trim() == studentId);
          return isAssigned;
        }).toList();
        ...
        if (courses.isEmpty && !isLoading) {
          return Center(
            child: Padding(
              ...
              child: Text(
                hasValidStudentId
                    ? 'لا توجد كورسات مسجلة لك في هذه الفرقة'
  ...
  Widget _buildGradeFilter() {
    // أربع فرق فقط – تظهر كلها بدون سحب أفقية. IDs: الأولى 44، الثانية 52، الثالثة 59، الرابعة 65
    const gradeOptions = [
      {'id': 44, 'name': 'أولى'},
      {'id': 52, 'name': 'ثانية'},
      {'id': 59, 'name': 'ثالثة'},
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Wrap: كل الخيارات ظاهرة بدون سحب (لا swipe left/right)
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: gradeOptions.map((grade) {
              final gradeId = grade['id'] as int;
              final isSelected = _selectedFiltersLevels == gradeId;
              return GestureDetector(
                onTap: () async {
                  setState(() {
                    _selectedFiltersLevels = gradeId;
                  });
                  final cubit = context.read<CoursesCubit>();
                  await cubit.getCourses(
                    refresh: true,
                    filtersLevels: gradeId,
                  );
                },
                child: Container(
